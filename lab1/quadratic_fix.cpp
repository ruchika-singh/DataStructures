...
int main() {
  ...
  //while (true) {
    for (int i = 0; i < 5; i++)
    {
      std::cout << "Enter 2 integer coefficients to a quadratic function: 1 + b*x + c = 0" << std::endl;
      int my_b, my_c;
      std::cin >> my_b >> my_c;
      b[i] = my_b;
      c[i] = my_c;
    ...
    for (int i = 0; i < 5; i++){
      float root_1, root_2;
      bool success = find_roots(1,b[i],c[i], root_1,root_2);
      ...
      if (success)
      {
        ...
        std::cout<<"The smaller root is: "<<smaller_root[i]<<std::endl;
      }
    }
      // Check our work...
      //if (check_root(1,b[i],c[i], root_1) && check_root(1,b[i],c[i], root_2)) {
        // Verified roots, break out of the while loop
        //break;
      //}
      //if (check_root(1,b[i],c[i], root_1) == false && check_root(1,b[i],c[i], root_2 == false))
      //{
